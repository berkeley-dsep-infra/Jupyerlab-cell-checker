...
async function fetchImageAsBase64(imageUrl) {
    if (imageUrl.startsWith("data:image")) {
        // If it is, strip the prefix and return the base64 data
        return imageUrl.split(",")[1];
    }
    const response = await axios__WEBPACK_IMPORTED_MODULE_6___default().get(imageUrl, { responseType: 'blob' });
...
async function sendImageToModel(imageUrl) {
    try {
        ...
        const response = await axios__WEBPACK_IMPORTED_MODULE_6___default().post("http://localhost:11434/api/generate", payload, {
...
async function checkMDNoAccessIssues(mdString, myPath, cellColor) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        while ((match = allImagesRegex.exec(mdString)) !== null) {
            ...
            // console.log(imageUrl); // prints out urls from md with no alt text
        }
...
class AltCellList extends _lumino_widgets__WEBPACK_IMPORTED_MODULE_3__.Widget {
    ...
    addCell(cellId, buttonContent) {
        ...
        // altText.style.marginLeft = '50px';
        // altText.style.marginRight = '50px';
        // altText.style.backgroundColor = 'white';
        // altText.style.border = '1px solid black';
        // altText.style.padding = '5px';
        // altText.textContent = "";
        var add = true;
    ...
    scrollToCell(cellId) {
        ...
        for (let i = 0; i < notebook.widgets.length; i++) {
            ...
            if (cell.model.id === cellId) {
                ...
                // if generated by external libraries -- prints out base 64
                // else -- prints out link
                const imageUrl = (_a = cell.node.querySelector('img')) === null || _a === void 0 ? void 0 : _a.src;
                sendImageToModel(imageUrl || " ");
...
//# sourceMappingURL=lib_index_js.6eaa79309ced5df31abc.js.map